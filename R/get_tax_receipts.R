...
#' UK taxes and duties from April 2016 to the most recent month (monthly
...
#'   to return all available series. Use [list_tax_heads()] to see valid
...
#'   locally and reused on subsequent calls. Use [clear_cache()] to reset.
#'
#' @return A data frame with columns:
#'   \describe{
#'     \item{date}{`Date`. The first day of the reference month.}
#'     \item{tax_head}{Character. Tax or duty identifier (see [list_tax_heads()]).}
...
get_tax_receipts <- function(tax = NULL, start = NULL, end = NULL, cache = TRUE) {
  ...
  if (!is.null(tax)) {
    ...
    if (length(bad) > 0) {
      cli::cli_abort(c(
        ...
        "i" = "Run {.fn list_tax_heads} to see valid values."
      ))
    }
  }
  ...
  url  <- resolve_govuk_url(slug, "NS_Table\\.ods")
...
parse_ns_table <- function(path) {
  # The data lives in the "Receipts_Monthly" sheet
  # Row 6 = column headers; rows 7+ = data ("April 2016", "May 2016", ...)
  ...
  # Parse dates from column 1: "April 2016" format
  ...
  for (j in seq_along(header_row)) {
    ...
    # Skip: date column, sub-components "(included within ...)", unmapped
    if (j == 1) next
