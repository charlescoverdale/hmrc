...
- scheme:
  ...
  `"sme"` (SME R&D Relief), `"rdec"` (Research and Development
  Expenditure Credit / large company scheme), `"total"` (combined).

- measure:

  Character vector or `NULL` (default = all measures). Valid values:
  `"claims"` (number of claims), `"amount_gbp_m"` (cost in millions of
  pounds).
...
#> ✔ Resolving download URL from GOV.UK Content API [321ms]
#> 
#> ℹ Downloading data file
#> ✔ Downloading data file [280ms]
#> 
#> ℹ Parsing data
#> New names:
#> • `` -> `...1`
